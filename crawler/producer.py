...
from crawler.tasks_crawler_tdx import crawler_tdx
...
for title in TDX_title_lists:
    print(f"預計抓{title}站點資料")
    # .delay() 是 Celery 的非同步派送捷徑, 呼叫完會立刻回傳, 不等 task 執行完
    crawler_tdx.delay(title=title, sub_url=TDX_title_dicts[title], parameter=TDX_title_dicts2[title])